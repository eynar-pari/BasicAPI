    ...
    public static boolean areEqualsJsonObject(String jsonExpected, String jsonActual) throws JSONException {
        ...
        while (keys.hasNext()) {
            ...
            if (valueExpected.equals("IGNORE")){
               Logger.getInstance().write("Ignoring.... Attribute : "+key+ " with value : "+valueActual ,"INFO");
            } else {
                if (!valueExpected.equals(valueActual)){
                    Logger.getInstance().write("Property: [" + key + "] -> Comparing Expected Result : '" + valueExpected +
                            "' and Actual Result : '" + valueActual + "'" + " are equal:" + valueExpected.equals(valueActual), "INFO");
